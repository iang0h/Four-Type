...
- If a provider call may have happened, the same attempt and key are retained for 24 hours from that timestamp, even below the ordinary 29-day reuse threshold. Reclaiming a released ambiguous attempt ETag-safely preserves both the timestamp and payload digest. A retry reconstructs the payload and must match that digest before transport. A digest mismatch from changed copy or configuration does not call Resend and returns a retryable ambiguity result; rotation is deferred until after the 24-hour window. Deterministic HMAC access signing is tested for the persisted session/expiry inputs.
